...
export const workspacesApiRef = createApiRef<WorkspacesApi>({
  id: 'plugin.workspaces.api',
});
...
export interface WorkspacesApi {
  ...
  getConfiguration(
    ...
    | { entityId: string; projectId: string; quickTemplateLink: string }
    | undefined
  >;
}
...
export class WorkspacesApiClient implements WorkspacesApi {
  ...
  me() {
    return this.call('/session/me');
  }

  async listWorkspaces() {
    return (await this.call('/workspaces')).workspaces;
  }
  async updateWorkspaceState(
    ...
    return await this.call('/update_state', {
      method: 'POST',
      body: JSON.stringify({
        projectId: +projectId,
  ...
  async getUserInfo() {
    return (await this.call('/user_info')).data;
  }
  async saveQuickLink(link: string, entityId: string, projectId: string) {
    return await this.call('/save_quick_link', {
      method: 'POST',
      body: JSON.stringify({
        link: link,
        entityId: entityId,
